...
INSERT INTO plugin_marketplaces(
  ...
  'nexus-marketplace.json', TRUE, TRUE, 'pending', CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
...
INSERT INTO plugin_resources_v2(
...
SELECT plugin_id, 'storage', 'r2', binding_name, external_name, TRUE, status,
       'preserve', '{"type":"r2","binding":"STORAGE"}'::jsonb, created_at,
       updated_at, preserved_at
  FROM plugin_runtime_resources
ON CONFLICT (plugin_id, logical_name) DO NOTHING;
...
ALTER TABLE plugins ADD COLUMN IF NOT EXISTS package_format INTEGER NOT NULL DEFAULT 1;
ALTER TABLE plugins ADD COLUMN IF NOT EXISTS marketplace_id TEXT;
ALTER TABLE plugins ADD COLUMN IF NOT EXISTS publisher_id TEXT;
ALTER TABLE plugins ADD COLUMN IF NOT EXISTS release_id TEXT;
ALTER TABLE plugins ADD COLUMN IF NOT EXISTS release_hash TEXT;
ALTER TABLE plugin_operations ADD COLUMN IF NOT EXISTS source_release_id TEXT;
ALTER TABLE plugin_operations ADD COLUMN IF NOT EXISTS package_format INTEGER NOT NULL DEFAULT 1;
ALTER TABLE plugin_operations ADD COLUMN IF NOT EXISTS assets_sha256 TEXT;

INSERT INTO permissions(id, key, created_at) VALUES
  ('perm_core_marketplace_read', 'core.marketplace.read', CURRENT_TIMESTAMP),
  ('perm_core_marketplace_create', 'core.marketplace.create', CURRENT_TIMESTAMP),
  ('perm_core_marketplace_update', 'core.marketplace.update', CURRENT_TIMESTAMP),
  ('perm_core_marketplace_delete', 'core.marketplace.delete', CURRENT_TIMESTAMP)
...
SELECT g.id, p.id, CURRENT_TIMESTAMP
 ...
 WHERE g.is_admin = TRUE AND p.key LIKE 'core.marketplace.%'
