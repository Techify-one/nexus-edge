...
import { resolveTranscriptionModel, } from "./transcription-models.js";
...
const auditStatement = (db, requestId, userId, action, resourceId, resourceType = "soletrando.child") => ({
...
export class SoletrandoRepository {
    ...
    async transcriptionSettings() {
        const row = await this.db.first(`SELECT transcription_model AS "transcriptionModel",updated_at AS "updatedAt"
    ...
    async updateTranscriptionSettings(transcriptionModel, userId, requestId) {
        ...
        await this.db.atomic([
            ...
            auditStatement(this.db, requestId, userId, "soletrando.settings.updated", "transcription", "soletrando.settings"),
        ]);
    ...
    async overview(search) {
        ...
        const [children, totals] = await Promise.all([
          ...
          ORDER BY c.created_at DESC`, [term, term]),
            this.db.first(`SELECT
    ...
    async createChild(name, userId, requestId) {
        const existing = await this.db.first("SELECT id FROM soletrando_children WHERE lower(name) = lower(?) LIMIT 1", [name]);
        if (existing)
            return null;
        ...
        await this.db.atomic([
            ...
            auditStatement(this.db, requestId, userId, "soletrando.child.created", id),
        ]);
        return { id, name, token, version: 1, createdAt: now, updatedAt: now };
    }
    async childDetail(id) {
        const child = await this.db.first(`SELECT id,name,token,version,created_at AS "createdAt",updated_at AS "updatedAt"
         FROM soletrando_children WHERE id = ?`, [id]);
        if (!child)
            return null;
        const [sessions, attempts, totals] = await Promise.all([
            this.db.query(`SELECT id,phase,status,started_at AS "startedAt",completed_at AS "completedAt",
                score,correct_count AS "correctCount",total_time_ms AS "totalTimeMs"
           FROM soletrando_sessions WHERE child_id = ?
          ORDER BY started_at DESC LIMIT 100`, [id]),
            this.db.query(`SELECT id,session_id AS "sessionId",phase,position,word,transcript,normalized,
          ...
          ORDER BY created_at DESC LIMIT 250`, [id]),
            this.db.first(`SELECT COUNT(*) AS "attemptsCount",
           ...
           FROM soletrando_attempts WHERE child_id = ?`, [id]),
        ]);
        return {
    ...
    async updateChild(id, name, version, userId, requestId) {
        const now = dbTime(this.db);
        const result = await this.db.atomic([
            ...
            auditStatement(this.db, requestId, userId, "soletrando.child.updated", id),
        ]);
        if (!result[0]?.rowsAffected)
            return null;
        return this.db.first(`SELECT id,name,token,version,created_at AS "createdAt",updated_at AS "updatedAt"
         FROM soletrando_children WHERE id = ?`, [id]);
    }
    async rotateToken(id, userId, requestId) {
        const token = randomToken();
        const result = await this.db.atomic([
            {
            ...
            auditStatement(this.db, requestId, userId, "soletrando.child.link_rotated", id),
        ]);
        return result[0]?.rowsAffected ? token : null;
    }
    async deleteChild(id, userId, requestId) {
        const result = await this.db.atomic([
            { sql: "DELETE FROM soletrando_children WHERE id = ?", params: [id] },
            auditStatement(this.db, requestId, userId, "soletrando.child.deleted", id),
        ]);
        return Boolean(result[0]?.rowsAffected);
    }
    async publicProfile(token) {
        const child = await this.db.first(`SELECT id,name,created_at AS "createdAt" FROM soletrando_children WHERE token = ?`, [token]);
        if (!child)
            return null;
        const [phaseResults, recentSessions, totals, activeSessions] = await Promise.all([
            ...
            ORDER BY completed_at DESC LIMIT 8`, [child.id]),
            this.db.first(`SELECT COUNT(*) AS "attemptsCount",
                  SUM(CASE WHEN is_correct = ${this.db.provider === "d1" ? "1" : "TRUE"} THEN 1 ELSE 0 END) AS "correctCount",
                  ROUND(AVG(total_score)) AS "averageScore"
             FROM soletrando_attempts WHERE child_id=?`, [child.id]),
            this.db.query(`SELECT id,phase,started_at AS "startedAt" FROM soletrando_sessions
            WHERE child_id=? AND status='active' ORDER BY started_at DESC LIMIT 1`, [child.id]),
        ]);
        const activeRow = activeSessions[0];
        let activeSession = null;
        if (activeRow) {
            const attempts = await this.db.query(`SELECT position,total_score AS "totalScore" FROM soletrando_attempts
          WHERE session_id=? ORDER BY position`, [activeRow.id]);
        ...
        return {
            ...
            phases: PHASES.map((phase) => {
                const result = phaseResults.find((row) => Number(row.phase) === phase.id);
                return {
                    id: phase.id,
                    title: phase.title,
    ...
    async childByToken(token) {
        return this.db.first("SELECT id,name FROM soletrando_children WHERE token=?", [token]);
    }
    ...
    activeSession(childId) {
        ...
        WHERE child_id=? AND status='active' ORDER BY started_at DESC LIMIT 1`, [childId]);
    }
    sessionProgress(sessionId) {
        return this.db.query(`SELECT position,total_score AS "totalScore" FROM soletrando_attempts
        WHERE session_id=? ORDER BY position`, [sessionId]);
    }
    async createSession(childId, phase) {
    ...
    sessionForToken(sessionId, token) {
        return this.db.first(`SELECT s.id,s.child_id AS "childId",s.phase,s.status,s.score,
        ...
        WHERE s.id=? AND c.token=?`, [sessionId, token]);
    }
    attemptExists(sessionId, position) {
        return this.db.first("SELECT id FROM soletrando_attempts WHERE session_id=? AND position=?", [sessionId, position]);
    }
    async saveAttempt(input) {
        const id = createId("attempt");
        await this.db.execute(`INSERT INTO soletrando_attempts(
    ...
    sessionAggregate(sessionId) {
        return this.db.first(`SELECT COUNT(*) AS "attemptCount",ROUND(AVG(total_score)) AS score,
              SUM(CASE WHEN is_correct = ${this.db.provider === "d1" ? "1" : "TRUE"} THEN 1 ELSE 0 END) AS "correctCount",
              SUM(elapsed_ms) AS "totalTimeMs"
         FROM soletrando_attempts WHERE session_id=?`, [sessionId]);
    }
    async finishSession(sessionId, score, correctCount, totalTimeMs) {
        const completedAt = dbTime(this.db);
        await this.db.execute(`UPDATE soletrando_sessions SET status='completed',completed_at=?,score=?,correct_count=?,total_time_ms=?
        WHERE id=? AND status='active'`, [completedAt, score, correctCount, totalTimeMs, sessionId]);
        return completedAt;
    }
}
